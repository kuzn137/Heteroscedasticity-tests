...
# Description
 ...
 Heteroscedasticity is tested by p value for resulting regression (if p value < 0.00001, we have heteroscedasticity). From experiments it was established that it is good to have p value not larger than 0.00001. Tests limitation is that it is assumed that resulting regression with residuals does not have Heteroscedasticity. If one method has high p value it is good to consider all methods.
